...
<style>
	...
	.textarea {
		...
		border-color: #d1d5db;
		color: black;
		padding: 0; /* paddingは親で制御するためリセット */
		border: none; /* 枠線も不要 */
		border-radius: 0;
		overflow: hidden;
		/* 背景を透明にして下のゴーストが見えないようにする */
	...
	.editing-ui {
		...
		/* 中身のtextareaとボタンをいい感じに配置 */
		display: flex;
		flex-direction: column;
	}

	.editing-ui .textarea {
		flex-grow: 1; /* 高さを親に合わせてめいっぱい伸ばす */
		min-height: 24px; /* 1行のときの最低限の高さ */
	}
	/* ▲▲▲ CSSの変更点はここまで ▲▲▲ */
</style>