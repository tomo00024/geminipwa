		...
		{#each currentSession.logs as message (message.timestamp)}
			<div class="chat-message-container {message.speaker === 'user' ? 'justify-end' : 'justify-start'}">
				<div class="chat {message.speaker === 'user' ? 'chat-end' : 'chat-start'}">
					<!-- ▼▼▼ CSS Gridのコンテナとして機能するように、HTMLはシンプルにします ▼▼▼ -->
...
<style>
	...
	/* ▼▼▼ CSS Gridによるレイヤー化のためのスタイル ▼▼▼ */
	.chat-bubble {
		position: relative; /* フローティングメニューのために必要 */
		max-width: 90%;
		padding: 0.5rem 1rem;
		border-radius: 1rem;
		...
		/* ここからが重要: グリッドコンテナ化 */
		display: grid;
		/* 子要素が全て同じ場所に配置されるようにする */
		grid-template-columns: 1fr;
		grid-template-rows: 1fr;
	}

	/* chat-bubble の直下の子要素すべてに適用 */
	.chat-bubble > * {
		/* 全ての子要素をグリッドの同じセル(1行目/1列目)に配置する */
		grid-column: 1 / -1;
		grid-row: 1 / -1;
	}

	/* 編集中に元のコンテンツを見えなくするが、場所は確保させる */
	.ghost-content {
		visibility: hidden;
	}
	...
	/* フローティングメニューは position:absolute のままなので z-index をつけて最前面に */
	...
	/* textareaのスタイルをシンプルに */
	.textarea {
		background-color: transparent; /* 背景は吹き出しの色を使う */
		color: black;
		padding: 0;
		border: none;
